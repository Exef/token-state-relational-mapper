...
from token_state_relational_mapper.mapper.database import Token, TokenHolder, get_session
...
def get_top_token_holders(contract_address, number_of_top):
    ...
    session = get_session()
    token_id, token_name, token_address, token_total_supply, token_decimals = session.query(Token.id, Token.name, Token.address, Token.total_tokens_supply, Token.decimals).filter_by(address=contract_address).first()

    holders = session.query(TokenHolder).filter(TokenHolder.held_token_id == token_id).order_by(desc(TokenHolder.balance)).limit(number_of_top).all()
